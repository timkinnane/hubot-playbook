...
var Improv = function (_Base) {
  _inherits(Improv, _Base);

  function Improv(robot) {
    var _ret;

    _classCallCheck(this, Improv);

    var _this = _possibleConstructorReturn(this, (Improv.__proto__ || Object.getPrototypeOf(Improv)).call(this, 'improv', robot));
...
function use(robot) {
  ...
  var searchStack = function searchStack(item) {
    return _lodash2.default.isEqual(item.toString(), middleware.toString());
  };
  if (improv.robot == null || _lodash2.default.findIndex(robot.middleware.response.stack, searchStack) < 0) {
  ...
  return _lodash2.default.map(strings, function (string) {
    ...
    while ((match = regex.exec(string)) !== null) {
      try {
        ...
        if (instance.config.replacement !== null) return instance.config.replacement;else string = string.replace(match[0], instance.config.fallback);
      }
    }
    return string;
  });
...
function middleware(context, next, done) {
  var hasExpression = _lodash2.default.some(context.strings, function (str) {
    return str.match(/\${.*}/);
  });
  if (hasExpression) {
...
function forget(path) {
  if (context == null) context = {};else _lodash2.default.unset(context, path);
  return this;
}
