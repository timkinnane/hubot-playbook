...
// init vars
var instance = void 0,context = void 0,extensions = void 0;
...
/**
                                                             ...
                                                             */var
Improv = function (_Base) {_inherits(Improv, _Base);
  function Improv(robot) {var _ret;_classCallCheck(this, Improv);var _this = _possibleConstructorReturn(this, (Improv.__proto__ || Object.getPrototypeOf(Improv)).call(this,
    'improv', robot));
    if (!instance) {
      instance = _this;
      _this.defaults({
...
/**
                                     ...
                                     */
function use(robot) {
  var improv = new Improv(robot);
  var searchStack = function searchStack(item) {return _lodash2.default.isEqual(item.toString(), middleware.toString());};
...
/**
   ...
   */
function configure() {var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
/**
  ...
  */
function mergeData() {var user = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  if (!instance) throw new Error('Improv must be used with robot before using data');
  if (context == null) context = {};
  var dataSources = [context, { user: user }];
  if (instance.config.save) dataSources.push(instance.robot.brain.get('improv'));
  var data = _lodash2.default.defaultsDeep.apply(_lodash2.default, [{}].concat(dataSources));
  var merged = _lodash2.default.reduce(extensions, function (merge, func) {
...
function parse(strings, data) {
  return _lodash2.default.map(strings, function (string) {
    var regex = new RegExp(/(?:\$\{\s?)(.*?)(?:\s?\})/);
    var match = void 0;
    while ((match = regex.exec(string)) !== null) {
      try {
        var template = new Function('return `' + match[0] + '`'); // eslint-disable-line
        // ⬆ StandardJS error: The Function constructor is eval
        var rendered = template.call(data);
...
function middleware(context, next, done) {
  var hasExpression = _lodash2.default.some(context.strings, function (str) {return str.match(/\${.*}/);});
  if (hasExpression) {
    var data = mergeData(context.response.message.user);
...

{
  ...
  get config() {if (instance) return instance.config;} };module.exports = exports['default'];
