  ...
  _createClass(Dialogue, [{
    ...
    */
    ...
    value: function receive(res) {
      ...
      res.dialogue = this;
      this.res = res;
      return this.path.match(res).then(function (result) {
        _this5.log.debug('Path match result: ' + _this5.res.match + ' (handler returned: ' + result + ')');
        if (_this5.res.match) _this5.clearTimeout();
        if (_this5.path.closed) _this5.end();
        return result;
      });
    }
  }]);
