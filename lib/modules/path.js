...
var Path = function (_Base) {
  ...
  function Path(robot) {
    ...
    var branches = _.isArray(args[0]) ? args.shift() : false;
    ...
    _this.closed = true;
    if (branches) {
      if (!_.isArray(branches)) _this.error('Branches must be Array');
  ...
  _createClass(Path, [{
    ...
    */
    ...
    value: function match(res) {
      ...
      var matchedBranch = _.find(this.branches, function (branch) {
