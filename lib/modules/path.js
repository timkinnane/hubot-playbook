...
var Path = function (_Base) {
  ...
  function Path(robot) {
    ...
    var branches = _lodash2.default.isArray(args[0]) ? args.shift() : false;

    var _this = _possibleConstructorReturn(this, (_ref = Path.__proto__ || Object.getPrototypeOf(Path)).call.apply(_ref, [this, 'path', robot].concat(args)));
    ...
    if (branches) {
      ...
      try {
        for (var _iterator = branches[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          _this.addBranch.apply(_this, _toConsumableArray(branch));
        }
      } catch (err) {
  ...
  */
  ...
  _createClass(Path, [{
    key: 'addBranch',
    value: function addBranch(regex) {
      var callback = void 0,
          strings = void 0;
      if (_lodash2.default.isString(regex) && _lodash2.default.isRegExp(new RegExp(regex))) regex = new RegExp(regex);
      if (!_lodash2.default.isRegExp(regex)) this.error('Invalid RegExp for branch: ' + regex);
    ...
    key: 'getHandler',
    value: function getHandler(strings, callback) {
      return function (res) {
        var sent = void 0,
            called = void 0;
        if (strings) {
          ...
          if (res.dialogue) sent = (_res$dialogue = res.dialogue).send.apply(_res$dialogue, _toConsumableArray(strings));else sent = res.reply.apply(res, _toConsumableArray(strings));
        }
