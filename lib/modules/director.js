...
*/
var Director = function (_Base) {
  _inherits(Director, _Base);

  function Director(robot) {
    var _ref;
    ...
    var authArg = _lodash2.default.isFunction(args[0]) ? args.shift() : null;

    var _this = _possibleConstructorReturn(this, (_ref = Director.__proto__ || Object.getPrototypeOf(Director)).call.apply(_ref, [this, 'director', robot].concat(args)));
    ...
    _this.log.info('New ' + _this.config.scope + ' Director ' + _this.config.type);

    var listEnv = _this.config.type.toUpperCase();
    switch (_this.config.scope) {
      case 'username':
        _this.names = process.env[listEnv + '_USERNAMES'];
        break;
      case 'room':
        _this.names = process.env[listEnv + '_ROOMS'];
    }
    if (_this.names != null) _this.names = _this.names.split(',');
    if (_this.names == null) _this.names = [];
    return _this;
  }
  ...
  */
  ...
  _createClass(Director, [{
    ...
    */
    ...
    key: 'process',
    value: function process(res) {
      var _this2 = this;

      var isAllowed = Promise.resolve(this.isAllowed(res));
      var user = res.message.user.name;
      var message = res.message.text;
      return isAllowed.then(function (allowed) {
        if (allowed) {
          _this2.log.debug(_this2.id + ' allowed ' + user + ' on receiving ' + message);
          _this2.emit('allow', res);
        } else {
          _this2.log.info(_this2.id + ' denied ' + user + ' on receiving: ' + message);
          _this2.emit('deny', res);
          if (!_lodash2.default.includes(['', null], _this2.config.deniedReply)) res.reply(_this2.config.deniedReply);
        }
        return allowed;
      });
    }
    ...
    */
    ...
    key: 'directMatch',
    value: function directMatch(regex) {
      var _this3 = this;
      ...
      this.robot.listenerMiddleware(function (context, next, done) {
        ...
        _this3.process(context.response).then(function (allowed) {
    ...
    */
    ...
    key: 'directScene',
    value: function directScene(scene) {
      this.log.info(this.id + ' now controlling ' + scene.id);
      var director = this;
      scene.registerMiddleware(function (context, next, done) {
        director.process(context.response).then(function (allowed) {
          if (allowed) next(done);else done();
        });
      });
      return this;
    }
